...
pub(crate) async fn command(
    ...
    let nodes_to_sample = if let Some(nodes_to_sample) = nodes_to_sample {
        SuiJsonValue::new(serde_json::json!([true, nodes_to_sample]))?
    } else {
        SuiJsonValue::new(serde_json::json!([false]))?
    };
