module atoma::db {
    ...
    public struct StackTrySettleEvent has copy, drop {
        ...
        leaf_stack_commitment: vector<u8>,
    ...
    public struct StackSettlementTicket has key, store {
        ...
        /// Corresponds to the `committed_stack_proof` in the `settlement` module.
        committed_stack_proof: vector<u8>,
        /// Corresponds to the `leaf_stack_commitment` in the `settlement` module.
        leaf_stack_commitment: vector<u8>,
    }
    ...
    /// * `leaf_stack_commitment` - A vector<u8> representing the commitment to the leaf of the stack's Merkle tree.
    /// * `random` - A reference to a Random object for generating random numbers.
    /// * `ctx` - A mutable reference to the transaction context.
    ///
    /// # Errors
    ...
    /// - `attestation_nodes`: A vector of SmallIds representing additional nodes sampled for attestation (if any).
    /// - `committed_stack_proof`: The cryptographic proof of the stack's execution.
    /// - `leaf_stack_commitment`: The commitment to the leaf of the stack's Merkle tree.
