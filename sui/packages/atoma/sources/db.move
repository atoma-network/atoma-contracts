module atoma::db {
    ...
    const InitialCrossValidationExtraNodesCount: u64 = 2;
    /// How many epochs after the vault expires during which any disputes must be resolved.
    ...
    const ENotVaultOwner: u64 = EBase + 26;
    const EVaultNotExpired: u64 = EBase + 27;
    const ENodeNotSelectedForVault: u64 = EBase + 28;
    const EVaultInDispute: u64 = EBase + 29;
    const ETooManyComputedUnits: u64 = EBase + 30;
    const ENotEnoughNodesEligibleForAttestation: u64 = EBase + 31;
    ...
    public struct VaultTrySettleEvent has copy, drop {
        small_id: SmallId,
        ...
        /// Corresponds to the `committed_vault_proof` in the `settlement` module.
        /// It is computed as a '2-dimensional' Merkle root of the each (input, output) pair
        /// that is processed through the vault, iterated over the range [0, CrossValidationExtraNodesCount).
        ...
        ///    b. We compute the root of the i-th Merkle tree root, as MH_vault(i).
        /// 2. Then compute the Merkle root of the `CrossValidationExtraNodesCount`-ary Merkle tree
        ///    whose leaves are the `CrossValidationExtraNodesCount` roots MH_vault(i), for i in [0, CrossValidationExtraNodesCount),
        ///    denoted as MH_vault.
        /// 3. The result MH_vault is then committed as `committed_vault_proof` (in big-endian bytes encoding).
        /// Note: We use Blake2b-256 as the cryptographic hash function H and Merkle tree hash function MH_vault.
        committed_vault_proof: vector<u8>,
        /// The leaf of the Merkle tree MH_vault(0), where 
        leaf_vault_commitment: vector<u8>,
    ...
    public struct VaultSettlementTicket has key, store {
        id: UID,
        /// The associated vault SmallId
        vault_small_id: SmallId,
        /// The node selected to process the requests in the vault
        selected_node_id: SmallId,
        /// Number of claimed computed units
        num_claimed_compute_units: u64,
        /// Nodes selected for attestation of the commitment output, either
        /// of size 0 or size `CrossValidationExtraNodesCount`, if the associated `Task` has security level set to `SamplingConsensus`.
        attestation_nodes: vector<SmallId>,
        /// The epoch after which the vault dispute is settled
        dispute_settled_at_epoch: u64,
        /// Corresponds to the `committed_vault_proof` in the `settlement` module.
        committed_vault_proof: vector<u8>,
        /// Corresponds to the `leaf_vault_commitment` in the `settlement` module.
        leaf_vault_commitment: vector<u8>,
    }
    ...
    /// # Arguments
    /// * `self` - A mutable reference to the AtomaDb object.
    /// * `wallet` - A mutable reference to the Coin<TOMA> object to pay for the vault.
    ...
    fun acquire_new_vault(
        ...
        let vault = Vault { 
            ...
            price,
            in_dispute: false,
            dispute_settled_at_epoch: option::none(),
            num_claimed_compute_units: option::none(),
        };
        ...
        let funds = wallet.split(price * num_compute_units);
    ...
    public entry fun try_settle_vault(
        self: &mut AtomaDb,
        node_badge: &mut NodeBadge,
        vault_small_id: u64,
        num_claimed_compute_units: u64,
        ...
        let vault_small_id = SmallId { inner: vault_small_id };
        let vault = self.vaults.borrow_mut(vault_small_id);
        assert!(vault.owner == ctx.sender(), ENotVaultOwner);

        let node_small_id = node_badge.small_id;
        if (node_small_id != vault.selected_node) {
            abort ENodeNotSelectedForVault
        };
        if (num_claimed_compute_units > vault.num_compute_units) {
            abort ETooManyComputedUnits
        };
        if (vault.in_dispute) {
            abort EVaultInDispute
        };

        let mut rng = random.new_generator(ctx);
        let attestation_nodes = self.sample_attestation_nodes(ctx, &mut rng);

        let ticket_id = object::new(ctx);
        let vault_settlement_ticked = VaultSettlementTicket {
            id: ticket_id,
            vault_small_id: vault_small_id,
            selected_node_id: node_small_id,
            num_claimed_compute_units,
            committed_vault_proof,
            leaf_vault_commitment,
            dispute_settled_at_epoch: ctx.epoch() + VAULT_DISPUTE_SETTLEMENT_DELAY,

        };

        vault.in_dispute = true;
        vault.dispute_settled_at_epoch = option::some(ctx.epoch() + VAULT_DISPUTE_SETTLEMENT_DELAY);
        vault.num_claimed_compute_units = option::some(num_claimed_compute_units);

        sui::event::emit(VaultTrySettleEvent {
            small_id: vault_small_id,
            num_claimed_compute_units,
            attestation_nodes,
            committed_vault_proof,
    ...
    ///   This ensures that vault settlement can proceed even with fewer attestation nodes than ideal.
    ///
    /// # Important
    ...
    public(package) fun sample_attestation_nodes(
        ...
        let attestation_nodes_count = InitialCrossValidationExtraNodesCount;
        ...
        if (vector::length(&eligible_nodes) < attestation_nodes_count) {
            ...
            // of aborting/throwing an error. Otherwise, the vault could not be claimed.
